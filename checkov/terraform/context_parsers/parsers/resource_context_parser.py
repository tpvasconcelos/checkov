...
class ResourceContextParser(BaseContextParser):
    ...
    def enrich_definition_block(self, definition_blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
        for entity_block in definition_blocks:
            ...
            if isinstance(entity_config[self.hcl2.START_LINE], list) and isinstance(entity_config[self.hcl2.END_LINE], list):
                entity_config[self.hcl2.START_LINE] = entity_config[self.hcl2.START_LINE][0]
                entity_config[self.hcl2.END_LINE] = entity_config[self.hcl2.END_LINE][0]
